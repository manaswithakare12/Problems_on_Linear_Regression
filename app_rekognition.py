...
def EmotionalAnalysis():
    """To do the emotional analysis of the selected images"""
...
def similarity(image_one, image_two):
    ...
    gray_image1 = cv2.cvtColor(
        cv2.resize(cv2.imread(image_one), (300, 300)), cv2.COLOR_BGR2GRAY
    )  # Read from disk
    # data2 image
    gray_image2 = cv2.cvtColor(
        cv2.resize(image_two, (300, 300)), cv2.COLOR_BGR2GRAY
    )  # Read from matrics
    # Cosine similarity
    similarity = cosine_similarity(
        gray_image1.reshape(1, -1), gray_image2.reshape(1, -1)
    )[0][0]
    # print(f"[info...] Similarity = {similarity}")
    return similarity


algo = lambda x: (x["CALM"] + x["HAPPY"])
...
def EngagementAnalysis(photo, outputjson_emotion, k, threshold=0.80):
    ...
    for i, value in enumerate(coordinates_emotions["FaceDetails"]):
        ...
        similarity_score = [
            similarity(
                file, cv2.cvtColor(roi[y_start:y_end, x_start:x_end], cv2.COLOR_BGR2RGB)
            )
        ...
        if max(similarity_score) >= threshold:
            ...
            output[
                "Image Link"
            ] = f"https://{Bucket_name}.s3.amazonaws.com/{Folder_in_S3}/{PersonName[similarity_score.index(max(similarity_score))]}_{photo_name}"
        else:
            output["Person"] = "XXXXX"
            output["Image Link"] = ""
    ...
    Result["Emotional Detection"] = Result.apply(algo, axis=1)
    Result["AI raw data"] = str(coordinates_emotions)
    if not os.path.exists(rootpath + "/db/out/excel"):
        os.makedirs(rootpath + "/db/out/excel")
    Result.to_excel(
        rootpath + "/db/out/excel/" + f"ImageAnalysis_{k}.xlsx", index=False
    )
...
def EngagementAnalysisBatch(threshold):
    ...
    photo_dir_files = dirFile(
        rootpath + "/db/input/videos/OutputDump/", ".jpg"
    )  # Selected image for analysis
    outputjson_emotion_dir_root = rootpath + "/db/out/"

    for k, filedir in enumerate(photo_dir_files):
        _, filename = os.path.split(filedir)
        EngagementAnalysis(
            filedir,
            outputjson_emotion_dir_root
            + filename.split(".jpg")[0]
            + "/"
            + jsonFileAlias
            + ".json",
            k,
...
def ExcelCombine():
    df = pd.DataFrame()
    files = dirFile(rootpath + "/db/out/excel", ".xlsx")
    for file in tqdm(files):
        df = df.append(pd.read_excel(file), ignore_index=True)

    df = df[df["Person"] != "XXXXX"]
    df1 = pd.DataFrame(
        df.groupby(["Person"])["Emotional Detection"].mean()
    ).reset_index(drop=False)
    df1 = df1[df1["Person"] != "XXXXX"]
    # list of dataframes and sheet names
    dfs = [df, df1]
    sheets = ["raw data combined", "Emotion Detection", "df2"]

    # run function
    if not os.path.exists(rootpath + "/out/"):
        os.makedirs(rootpath + "/out/")
    dfs_tabs(dfs, sheets, rootpath + "/out/" + "EmotionDetection.xlsx")
...
if __name__ == "__main__":
    ...
    uploadIn = int(
        input(
            "\nDo you want to automatically create master images? [Press 1 for Yes and 0 for No (default 1)]:"
        )
        or "1"
    )
    ...
    if uploadIn == 1:
        ...
        permission = int(
            input(
                f"\n[Alert!!..] Please confirm you have transfered one representable file from {ImageCollection} to {rootpath}/db/masterImg/ manually ! [Press 1 for Yes and 0 for No (default 1)]"
            )
        )
        ...
        try:
            ...
            reko(
                dirFile(rootpath + "/db/masterImg/", ".jpg")[0],
                rootpath + "/db/masterImg/",
            )
            facedetect(
                dirFile(rootpath + "/db/masterImg/", ".jpg")[0],
                dirFile(rootpath + "/db/masterImg/", ".json")[0],
            )
            upload_folder_to_s3(
                s3.Bucket(Bucket_name), rootpath + "/db/artifact/", Folder_in_S3
            )

        except:
            print(
                f"Error Occured !! it may be due to slow internet or you have not transfered the file from {ImageCollection} to {rootpath}/db/masterImg/ manually"
            )
    else:
    ...
    entry = int(
        input(
            "\nDo you want to proceed for Emotional analysis using AWS? [Type 1 for yes and 0 for No (default 1)]"
        )
        or "1"
    )
    ...
    entry2 = int(
        input(
            "\nDo you want to proceed for Emotion detection analysis? [Type 1 for yes and 0 for No (default 1)]"
        )
        or "1"
    )

    if entry2 == 1:
        ArtifactFiles = glob.glob(
            rootpath + "/db/artifact/Person_*.jpg"
        )  # get all the file name with directory ends with *.jpg
        PersonName = list(
            map(lambda x: os.path.split(x)[1].split(".")[0], ArtifactFiles)
        )  # To get the name of the persons
